...
| 근접 광각 추적 카메라 | Arducam OV9281 글로벌셔터 MIPI 카메라 (120°+ 광각렌즈) | 100,000 | 롤링셔터 왜곡(고속 이동 시 화면 일그러짐) 방지용 **글로벌셔터 필수**. 고속 급기동 중 표적이 화면 밖으로 이탈하지 않도록 광시야각 확보 + NPU 디지털 크롭으로 추적. ⚠️ **디지털 줌은 해상도를 만들어내지 못해 장거리 탐지(위 항목) 역할 대체 불가** — 근접 구간 전용. 가격/광각렌즈 옵션/JetPack6 드라이버 지원은 arducam.com 접근 차단으로 이번 조사에서 미검증(추정치). 🔍 **검토 후 기각(2026-09)**: Arducam 12MP IMX708 광각 모듈은 **롤링셔터 센서로 확인**(Arducam 공식 문서), Jetson 네이티브 지원도 미확인 — 글로벌셔터 원칙 위반으로 채택 안 함, OV9281 유지 |
...
6. **250km/h 목표 검증** — 확정된 6035(3.5피치) 프롭 실측 후 고피치 대안과 비교, 장거리 탐지 카메라 세트 실가격/무게 최종 확인 필요
